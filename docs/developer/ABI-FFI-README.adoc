...
== EPISTEMICTYPES.JL ABI/FFI Documentation
...
│  generated/abi/epistemictypes.jl.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libepistemictypes.jl.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── epistemictypes.jl.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── epistemictypes.jl.h       # Generated from Idris2 ABI
│
...
----
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/epistemictypes.jl.h
----
...
[source,c]
----
#include "epistemictypes.jl.h"

int main() {
    void* handle = epistemictypes.jl_init();
    if (!handle) return 1;

    int result = epistemictypes.jl_process(handle, 42);
    if (result != 0) {
        const char* err = epistemictypes.jl_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    epistemictypes.jl_free(handle);
    return 0;
}
----
...
[source,bash]
----
gcc -o example example.c -lepistemictypes.jl -L./zig-out/lib
----
...
[source,idris]
----
import EPISTEMICTYPES.JL.ABI.Foreign
...
[source,rust]
----
#[link(name = "epistemictypes.jl")]
extern "C" {
    fn epistemictypes.jl_init() -> *mut std::ffi::c_void;
    fn epistemictypes.jl_free(handle: *mut std::ffi::c_void);
    fn epistemictypes.jl_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = epistemictypes.jl_init();
        assert!(!handle.is_null());

        let result = epistemictypes.jl_process(handle, 42);
        assert_eq!(result, 0);
...
[source,julia]
----
const libepistemictypes.jl = "libepistemictypes.jl"

function init()
    handle = ccall((:epistemictypes.jl_init, libepistemictypes.jl), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:epistemictypes.jl_process, libepistemictypes.jl), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:epistemictypes.jl_free, libepistemictypes.jl), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/epistemictypes.jl.h
   ```
