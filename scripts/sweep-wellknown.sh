...
Classify, migrate and test repositories in batches (EpistemicTypes.jl#119).
...
sweep_repo() { # repo class -> appends to RESULT_CSV
    ...
    if [ "$DRY_RUN" -eq 0 ] && [ "$status" != failed ]; then
        if [ -n "$(cd "$dir" && git status --porcelain)" ]; then
            ...
            if (cd "$dir" && git add -A && git commit -qm "chore(www): migrate root .well-known/ to www/.well-known/ (EpistemicTypes.jl#119)"); then
                if [ "$PUSH" -eq 1 ]; then
