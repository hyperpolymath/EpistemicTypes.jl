...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "EpistemicTypes.jl"`
...
# an instantiation. The template submitted the literal `EpistemicTypes.jl` as its own
...
# is set to `owner/EpistemicTypes.jl` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `EpistemicTypes.jl-2`,
